...
class IrSequence(models.Model):
    ...
    def _is_inter_transit_protected_ir_sequence(self, operation_type='write', vals=None):
        """
        ...
        protected_sequence_ids = self.env['ir.sequence'].search(
            [('code', 'in', ['transit.order', 'transit.picking'])]
        ).mapped('id')
    ...
    def write(self, vals):
        if self.env.context.get('skip_ir_sequence_write_protection'):
            restricted_fields = {'active', 'code', 'company_id', 'name'}
            if any(field in vals for field in restricted_fields):
                self._is_inter_transit_protected_ir_sequence('write', vals)
        return super().write(vals)

    def unlink(self):
        if self.env.context.get('skip_ir_sequence_unlink_protection'):
            self._is_inter_transit_protected_ir_sequence('unlink')
        return super().unlink()